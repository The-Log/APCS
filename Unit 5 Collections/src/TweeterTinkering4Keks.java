...
class twitterFun {
    ...
    public void bot() throws TwitterException, IOException, InterruptedException{
        ...
        String username = JOptionPane.showInputDialog("Enter user, who you wish to automate messages");
        List<String> messages = new ArrayList();
        String message = JOptionPane.showInputDialog("Enter a couple messages. If you are finished type \"done.\"");
        while (!message.equals("done")) {
            messages.add(message);
            message = JOptionPane.showInputDialog("Enter a couple messages. If you are finished type \"done.\"");
        }
        String prev = "";
        String prior = "";
        ...
        while (true) {
            ...
            messages.set(message.indexOf(current), current + "!!!");
            while (current == prev || current == prior) {
                current = messages.get((int) (messages.size() * Math.random()));
                messages.set(message.indexOf(current), current + "!!!");
            }
            StatusUpdate statusUpdate = new StatusUpdate("@" + username + " " + current);
            Status status = twitter.updateStatus(statusUpdate);
            System.out.println("Please wait till 5 mins.");
